...
export default function Step9({ updateFunc, alrdyPickedGods }) {
    ...
    return (
        <div className="flex flex-col h-92% mt-1">
            <h1 className="text-2xl text-center text-white">Order Second Pick {lock1 ? "Done" : ""}</h1>
            {lock1 ? <></> : <GodList pickedGods={alrdyPickedGods} lockInGod={lockIn} hover={godHover} />}
            <h1 className="text-2xl text-center text-white">Order Third Pick {lock2 ? "Done" : ""}</h1>
